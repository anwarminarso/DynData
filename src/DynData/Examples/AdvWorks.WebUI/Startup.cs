...
using AdvWorks.WebUI.Data;
using Microsoft.AspNetCore.Identity;

namespace AdvWorks.WebUI
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            AdvWorksDbContext.Open(settings.DBConnectionSetting, db =>
            {
...
Database Name   : {1}", settings.DBConnectionSetting.Provider.ToString(), db.Database.GetDbConnection().Database);
            ...
            services.AddDbContext<AdvWorksDbContext>(o =>
            {
                // without custom event handler
                //o.UseDynData(settings.DBConnectionSetting);
                
                // with custom event handler
                o.UseDynData(settings.DBConnectionSetting, new EventHandlers.AdvWorksDbContextHandler());
            });
            #region Compression
            ...
            // enable DynData API
            //services.AddDynDataApi<AdvWorksDbContext>("tableOnly"); // without query template ==> api path /dyndata/tableOnly/...
            
            
            services.AddDynDataApi<AdvWorksDbContext, AdvWorkQueryTemplate>("adv"); // with query template ==> api path /dyndata/adv/...

            //services.AddDynDataApi<Security.APIAuth, AdvWorksDbContext, AdvWorkQueryTemplate>("adv"); // with auth and query template ==> api path /dyndata/adv/...
        }
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, AppSettings settings, IServiceScopeFactory factory)
        {
            ...
            //app.RegisterDynDataServiceAPIAuth<Security.APIAuth, AdvWorksDbContext, AdvWorkQueryTemplate>("adv");
